"""
OllamaSwarm — webscout port of g4f/Provider/OllamaSwarm.
...
``text/event-stream`` reply the same way every other webscout streaming
provider does.
...
* Results are cached per-day under
  ``~/.cache/webscout/ollama_swarm/<YYYY-MM-DD>/servers.json`` so the
...
def _cache_dir() -> Path:
    ...
    p = Path.home() / ".cache" / "webscout" / "ollama_swarm" / date.today().strftime("%Y-%m-%d")
...
class OllamaSwarm(Provider):
    ...
    def ask(
        ...
        **kwargs: Any,
        ...
        # pair (the standard webscout pattern). Using ``yield`` directly
