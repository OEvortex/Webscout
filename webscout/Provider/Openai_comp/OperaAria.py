...
Mirrors :class:`webscout.Provider.OperaAria` (the regular webscout provider)
...
``webscout/Provider/OperaAria.py`` for the full reverse-engineering notes
(anonymous 3-step OAuth signup, v1/v2 endpoints, SSE response format).

Differences from the regular webscout ``OperaAria``:
...
class _OperaAriaState:
    ...
    can sit on the webscout client instance without dragging in any g4f
    types.
...
class OperaAria(OpenAICompatibleProvider):
    ...
    # ── auth (mirrors webscout/Provider/OperaAria.py) ─────────────
