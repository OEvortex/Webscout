...
Install or update LLM4Free to get access to the TempMail module:
...
```python
from llm4free.Extra.tempmail import get_random_email
...
```python
from llm4free.Extra.tempmail import get_provider
...
```bash
python -m llm4free.Extra.tempmail.cli --help
```
...
python -m llm4free.Extra.tempmail.cli --wait 15
```
...
### Asynchronous Utilities (`llm4free.Extra.tempmail.async_utils`)
...
```python
import asyncio
from llm4free.Extra.tempmail.async_utils import get_temp_email, wait_for_message
...
Adding support for a new temporary email service is straightforward thanks to the abstract provider pattern used throughout LLM4Free:

1.  **Create Provider Classes**: Implement two new classes:
    *   One inheriting from `llm4free.Extra.tempmail.base.TempMailProvider` for synchronous operations.
    *   One inheriting from `llm4free.Extra.tempmail.base.AsyncTempMailProvider` for asynchronous operations.
...
2.  **Register in Factory**: Update the `get_provider` function in `llm4free/Extra/tempmail/base.py` to recognize a new `provider_name` string and return instances of your newly created classes.
3.  **Expose (Optional)**: If desired, update `llm4free/Extra/tempmail/__init__.py` to directly import and expose your new provider classes for users who might want to instantiate them directly.
...
*   **Parser**: Uses the `Scout` HTML/XML parser from the core `llm4free` library instead of external libraries like `BeautifulSoup` for consistency and potentially better performance within the LLM4Free ecosystem.
...
*   **Minimal Dependencies**: Avoids adding external logging or unnecessary dependencies to ensure smooth integration into projects using LLM4Free.
...
Please refer to the main LLM4Free project's contributing guidelines if you plan to submit a pull request.
...
<div align="center">
  ...
  <p>📧 TempMail - Part of the LLM4Free Toolkit</p>
  <a href="https://github.com/pyscout/LLM4Free">Back to Main LLM4Free Project</a>
</div>