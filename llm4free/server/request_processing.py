...
async def log_api_request(
    request_id: str, ip_address: str, model: str, question: str, answer: str, **kwargs
) -> bool:
...
def prepare_provider_params(
    ...
    messages = (
        normalize_text_messages(processed_messages) if use_text_only_compat else processed_messages
    )
                                ...
                                openai_content.append(
                                    {
                                        "type": "image_url",
                                        "image_url": {
                                            "url": f"data:{media_type};base64,{block.source.data}"
                                        },
                                    }
                                )
...
def convert_openai_to_anthropic_response(
    openai_response: Dict[str, Any], anthropic_model: str
) -> Dict[str, Any]:
...
def convert_openai_chunk_to_anthropic_events(
    chunk: Dict[str, Any], anthropic_model: str, is_first: bool = False
) -> List[Dict[str, Any]]:
    ...
    for choice in choices:
        ...
        for i, tc in enumerate(tool_calls):
            if tc.get("type") == "function":
                ...
                events.append({"type": "content_block_stop", "index": i + 1})
